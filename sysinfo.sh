...
SI="echo ${b}Hostname${n}: $(hostname) \
    ${b}• OS${n}: $(cat /etc/lsb-release | grep DESCRIPTION | cut -c 22- | head -c -2) \
    ${b}• CPU${n}: $(cat /proc/cpuinfo | grep cores | tail -n1 | awk '{ print $4 }')x $(cat /proc/cpuinfo | grep model | tail -n1 | awk '{$1=$2=$3=""; print $0}') \
    ${b}• RAM${n}: $(echo $(grep MemTotal /proc/meminfo | awk '{print $2}') / 1000000 | bc)GB \
    ${b}• Storage${n}: $(df | grep '^/dev/[hs]d' | awk '{s+=$2} END {print s/1048576}')GB Total ($(df | grep '^/dev/[hs]d' | awk '{s+=$4} END {print s/1048576}')GB Free) \
    ${b}• VGA${n}: $(lspci | grep VGA | awk '{$1=$2=$3=$4=""; print $0}') ${b}• Uptime${n}: $(uptime --pretty | cut -c 4-)" \
